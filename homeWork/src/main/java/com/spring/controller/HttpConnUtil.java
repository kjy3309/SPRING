...
public class HttpConnUtil {
	...
	public void sendMsg() {
		...
		/*
		 * if(method.equals("GET") && params != null) { urls.add("?"+params); }
		 */
		...
		try {
			...
			*/
			...
			/*
			 * if(header != null && header.size()>0) {//헤더 값이 있을 경우 추가 해 준다. for(String key
			 * : header.keySet()) { con.setRequestProperty(key, header.get(key)); } }
			 */		
			...
			if(con.getResponseCode()==200) { // 요청이 성공적으로 되었는가?
		        ...
		        for(Key k : key) {
		        	String name = k.stationName.substring(0,k.stationName.indexOf("."));
		        	dao.bike(name, k.parkingBikeTotCnt);
		        }
		        sendMsg2();
	...
	public void sendMsg2() {
		String method = "GET";
		ArrayList<String> urls = new ArrayList<String>();
		urls.add("http://openapi.seoul.go.kr:8088/765663435864686433384657724e46/json/bikeList/1000/1051/");
